...
public class HashMap<K, V> implements Map<K, V> {
    ...
    private final float DEFAULT_LOAD_FACTOR = 0.75f; // 当size > 0.75 * table.length是扩容
    ...
    private void resize() {
        ...
        for (int i = 0; i < oldTable.length; i++) {
            ...
            while (!queue.isEmpty()){
                ...
                // 注意: moveToTable()会将node内部的所有指针清空，所以写在左右子节点入队后
                moveToTable(node);
            }
        }
    }

    private void moveToTable(Node<K,V>node){
        // 重置node中的指针
        node.left = null;
        node.right = null;
        node.parent = null;
        // 特别注意: 扩容时将节点当做新添加的节点，新添加的节点应该为RED
        node.color = RED;

        int index = index(node); // TODO: 可以优化
            ...
            while (cur != null) {
                ...
                if (h1 < h2) {
                    ...
                    res = 1;  // 标记为 向左查找 cur = cur.right;
                        ...
                        (res = ((Comparable<K>) k1).compareTo(k2)) != 0) { // 具备可比较性
                } else {
                    // 扩容肯定没有相等的key,使用地址确定前进方向
